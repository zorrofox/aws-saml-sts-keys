...
async function onBeforeRequestEvent(details) {
  ...
  chrome.storage.sync.get({
    ...
    if(item.FileOrText==="text") {
      outputDocAsTextarea(credentials);
    }
    else {
...
function outputDocAsTextarea(docContent) {
  let textContent = changeEnvExport(docContent.split(LF).slice(1)).join(LF).replace();
  console.log(textContent);
  chrome.storage.sync.set({AccessKeyConetent: textContent});
}

function changeEnvExport(arr) {
  arr[0] = arr[0].replace("aws_access_key_id", "export AWS_ACCESS_KEY_ID");
  arr[1] = arr[1].replace("aws_secret_access_key", "export AWS_SECRET_ACCESS_KEY");
  arr[2] = arr[2].replace("aws_session_token", "export AWS_SESSION_TOKEN");
  return arr;
}
